...
public abstract class MinMaxAlbatar {
     ...
     */
    public MinMaxAlbatar(Board board, int depth, double alpha, double beta) {
            ...
            if (board.getPlayerHoles () [i] != 0)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (depth < MinMaxAlbatar.maxDepth)
                        {
                            /* On construit le noeud suivant */
                            MinMaxAlbatar child = this.getNextNode (copy, depth + 1, alpha, beta);
    ...
    {
        MinMaxAlbatar.maxDepth = maxDepth;
        MinMaxAlbatar.player = board.getCurrentPlayer ();
    }

    private int diffScore (Board board)
    {
        int score = 0;
        boolean isUnderThree = false;
        if(MinMaxAlbatar.player==board.getCurrentPlayer()){
            for(int i = 0; i<Board.NB_HOLES; i++){
                if(board.getOpponentHoles()[i]<3){
                    if(isUnderThree){
                        score *= (6-i);
                    }
                    score+=board.getOpponentHoles()[i];
                    isUnderThree = true;
                }else{
                    isUnderThree = false;
                }
     ...
     */
    protected abstract MinMaxAlbatar getNextNode (Board board, int depth, double alpha, double beta);
