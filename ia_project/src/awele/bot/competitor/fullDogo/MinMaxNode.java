...
public abstract class MinMaxNode
{
	                ...
	                try
	                {
	                    ...
	                    else
	                    {
	                        ...
	                        /* Sinon (si la profondeur maximale est atteinte), on évalue la situation actuelle */
	                            ...
	                            this.decision [i] = this.evalSituation(i,copy, depth);
	                    }
     ...
     * @param minDepth 
     */
    protected static void initialize (Board board, int minDepth)
    {
    	MinMaxNode.maxDepth = minDepth + (int)Math.round(48.0/board.getNbSeeds());
    ...
    private double evalSituation(int coupJoue, Board board, int depth)
    { 	
        return diffScore(board) + EvalSituation.evalSituation(board, depth);   
    }
